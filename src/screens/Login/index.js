      ...
      if (nextProps && nextProps.responseCode == ERROR_CODE.SUCCESS) {
        switch (nextProps.actionType) {
          case CHECK_LOGIN_SUCCESS:
            registerUser(nextProps.shopInfo.user_name, this.state.password, this.state.email, nextProps.shopInfo.shop_id, nextProps.shopInfo.shop_email, Constant.getUniqueId(), "1", this.state.myToken).then(({ result, message }) => null);
            this.loginLocal()
            break;
          default:
            break;
