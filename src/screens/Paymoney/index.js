...
import { saveTransaction } from '../../actions/TransactionAction'
import { BluetoothEscposPrinter, BluetoothManager } from 'react-native-bluetooth-escpos-printer';
import ViewShot from "react-native-view-shot";
import RNFS from 'react-native-fs';
import { SAVE_TRANSACTION } from '../../constants/type'
...
class Paymoney extends Component {
    ...
    componentDidMount() {
        if (this.props.dataPrinter) {
            BluetoothManager.connect(this.props.dataPrinter.boundAddress)
                .then((s) => {
                    // alert("Success");
                    ...
                    this.setState({isLoading: false})
                    Alert.alert(I18n.t('error'), I18n.t('noConnectPrinter'))
                })
        }
        console.log('dataPrinter', this.props.dataPrinter)
        if(this.props.dataPrinter == null){
            this.setState({isLoading: false})
        }
    }
    ...
    onPressToSave = async () => {
        ...
        try {
            ...
            let data = {
                ...
                invoiceStatus: status == 1 ? I18n.t('paid') : I18n.t('notPaid')
            }
            ...
            // this.props.onSaveTransaction(data)
        } catch (error) {
            this.setState({ isLoading: false })
            alert(error)
        }
        ...
        if (result && result.length > 0) {
            ...
            if (trans_state == 0) {
                ...
                if (result[0].status == 1) {
                    checkOutTransaction(moment(new Date()).format("HH:mm:ss"), trans_id, this.props.login.shop_id, this.props.login.shop_email).then(({ result, message }) => this.resultCheckOut(result));
                            ...
                            <View style={styles.titleInput}>
                                ...
                                <View style={[styles.inputVehicle, { flexDirection: 'row' }]}>
                                    <TextInput value={this.state.vehicle_no} placeholder={'ກກ 0000'} maxLength={7}
                                        ...
                                        // onSubmitEditing={()=> this.onPressToSave()}
                                    />
                        ...
                        <View style={styles.modalContent}>
                            ...
                            <View style={{ marginLeft: 15, marginRight: 15 }}>
                                <View style={styles.search}>
                                    <TextInput autoFocus={false} placeholder='Enter text search ...' style={{ marginLeft: 10 }} onChangeText={(text) => this.onSearch(text)} />
                                </View>
                            </View>
...
const mapStateToProps = state => {
    return {
        ...
        dataPrinter: state.trans.dataPrinter,
        dataTransaction: state.trans.dataTransaction,
...
const mapDispatchToProps = dispatch => {
    return {
        onSaveTransaction: (data) => dispatch(saveTransaction(data)),
    };
};
