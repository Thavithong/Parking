...
import { getTransReturn, updateTransaction, insertReturnMoney } from '../../controller/TransactionController';
...
class ReturnMoney extends Component {
    ...
    initGetTransaction = () => {
        ...
        getTransReturn(this.props.login.shop_id, this.props.login.shop_email).then(({ result, message }) => this.setData(result));
    }
    ...
    onPressSave = () => {
        ...
        updateTransaction(3, this.state.trans_id, this.props.login.shop_id, this.props.login.shop_email).then(({ result, message }) => null);
        insertReturnMoney(this.state.vehicle_id, this.state.vehicleNumber, this.props.login.user_id, this.state.amount, moment(new Date()).format("YYYY-MM-DD hh:mm:ss A"), this.props.login.shop_id, this.props.login.shop_email).then(({ result, message }) => null);
                ...
                <View style={styles.container}>
                    {visibleSearch == false ?
                        <Header
                            ...
                            centerComponent={{ text: I18n.t('return_money'), style: styles.head_text }}
                        ...
                        <View style={styles.modal}>
                            <View style={styles.modalContent}>
                                ...
                                <View style={styles.content} >
                                    <Text style={[styles.textContent, { fontWeight: '500' }]}>{I18n.t('askRevertMoney')}</Text>
...
export default connect(mapStateToProps, null)(ReturnMoney);