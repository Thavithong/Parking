...
function* callShopInfo(action) {
    try {
        ...
        console.log('return form server : ', item)
        if (item.status === 200) {
            let response_code = item.data.response_code
            let response_description = item.data.response_description
            let status = item.data.status
            let filed = item.data.fields
            if (response_code == ERROR_CODE.SUCCESS && status == true) {
                yield put({ type: CHECK_SHOP_INFO_SUCCESS, shopInfo: item.data.fields, responseCode: response_code, responseDescription: response_description })
            } else {
                yield put({ type: CHECK_SHOP_INFO_FAILED, shopInfo: null, responseCode: response_code, responseDescription: response_description })
            }
        } else {
