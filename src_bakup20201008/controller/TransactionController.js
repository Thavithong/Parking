        ...
        db.transaction((tx) => {
            tx.executeSql('SELECT * FROM tb_transaction where trans_date >= ? and shop_id=? and shop_email=? order by trans_id desc', [date,shop_id, shop_email], (tx, results) => {
                for (let i = 0; i < results.rows.length; i++) {
                    ...
                    let transaction = new Transaction(item.trans_id, item.trans_date, item.amount, item.vehicle_id, item.vehicle_name, item.vehicle_number, item.user_id, item.status, item.trans_time, item.shop_id, item.shop_email);
...
export const checkOutTransaction = (trans_id, shop_id, shop_email) => {
    return new Promise((resolve, reject) => {
        let msg = new Message();
        msg.result = [];
        var query = "update tb_transaction set trans_sate=0 where trans_id=? and shop_id=? and shop_email=?";
        var param = [trans_id, shop_id, shop_email]
        db.transaction((tx) => {
            tx.executeSql(query, param, (tx, results) => {
                let data = {
                    status: true,
