    ...
    render() {
        return (
            <View style={styles.container}>
                ...
                // rightComponent={<LeftComponent iconSize={35} icons_name='ios-save' onPress={() => { this.onPressToServer() }} />}
                />
                {this.state.isLoading ? <ActivityIndicator /> : null}
                <View style={styles.content}>
                    <View style={styles.group}>
                        {/* <View style={[styles.groupMainRow, { borderTopWidth: 0 }]}>
                            <View style={[styles.groupRow, { borderWidth: 0 }]}>
                                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    <Image source={images.iconServer} resizeMode='contain' style={{ marginLeft: 20, width: 40, height: 40 }} />
                                    <Text style={[styles.groupText, { marginLeft: 10 }]}>{I18n.t('firstServer')}</Text>
                                </View>
                                <View style={{ marginRight: 10, justifyContent: 'center' }}>
                                    <ToggleSwitch
                                        isOn={this.state.isToggleOnline}
                                        onColor={colors.buttonColor}
                                        offColor={colors.toggleColor}
                                        labelStyle={{ color: "black", fontWeight: "20", }}
                                        size="medium"
                                        onToggle={isOn => this.setValueOnline(!this.state.isToggleOnline)}
                                    />
                                </View>
                            </View>
                            {this.state.isToggleOnline ?
                                <View style={{ backgroundColor: colors.white, paddingLeft: 20, paddingRight: 10, justifyContent: 'center', }}>
                                    <MyTextInput
                                        ...
                                        onChangeText={(text) => this.setState({ publicIp: text })}
                                    />
                                </View>
                                : null}
                        </View> */}
